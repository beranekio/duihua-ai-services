...
After a local gateway image rebuild, `scripts/build-and-load-images.sh` and `scripts/deploy-kind.sh` restart the gateway Deployment so running pods load the new image even when Helm reuses the same `GATEWAY_IMAGE_TAG` (default `local`). Set `GATEWAY_ROLLOUT_RESTART=false` to skip automatic restarts.
