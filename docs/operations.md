...
This path does not change inference Deployments or vLLM flags. Tune `gateway.responsesApiStore.backgroundJobs.streamKey` and `staleSeconds` for queue routing and stale-response reconciliation on `GET /v1/responses/{id}`. Stream retention (trim after `XACK`) is handled by the background-worker consumer, not gateway `XADD`.
