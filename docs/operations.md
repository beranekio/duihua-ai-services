...
- **Legacy redis-streams driver:** set `backgroundWorker.autoscaling.driver: redis-streams` to scale from Valkey consumer-group lag directly (requires Valkey/Redis 7+). Use `gateway.responsesApiStore.redisAddress` when bundled Valkey is disabled. `lagCount` and `activationLagCount` remain as deprecated aliases.
