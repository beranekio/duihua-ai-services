...
Enable KEDA autoscaling with `backgroundWorker.autoscaling.enabled=true`. By default (`driver: store-metrics`), the chart renders a `ScaledObject` that queries the responses-api-store HTTP endpoint `GET /metrics/background-queue?consumer_group=<backgroundWorker.consumerGroup>` and scales on the JSON `workload` field (`pending + in_progress`). This uses KEDA's `metrics-api` scaler; only KEDA core is required (not the HTTP add-on). KEDA does not connect to Valkey directly.

- **Scale-to-zero:** set `backgroundWorker.autoscaling.replicas.min` to `0`. Keep `activationTargetValue` at `0` so a single queued job activates scaling. Tune `jobsPerReplica` if brief idle periods flap replicas.
- **Store metrics:** enable `responses-api-store.metrics.enabled=true` (default) so the store Service exposes port `8080` (or `responses-api-store.metrics.port`). When the store subchart is disabled, set `backgroundWorker.autoscaling.metricsUrl` to a KEDA-reachable URL.
...
- **Legacy redis-streams driver:** set `backgroundWorker.autoscaling.driver: redis-streams` to scale from Valkey consumer-group lag directly (requires Valkey/Redis 7+). With bundled Valkey disabled while the store subchart stays enabled, the scaler still targets the chart-managed Valkey Service name unless issue #58 is addressed; prefer `store-metrics` for external Redis or disable the subchart and set `gateway.responsesApiStore.redisAddress`. `lagCount` and `activationLagCount` remain as deprecated aliases.
