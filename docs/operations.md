...
This path does not change inference Deployments or vLLM flags. Tune `gateway.responsesApiStore.backgroundJobs.streamKey`, `staleSeconds`, and `streamMaxLen` for queue retention and stale-response reconciliation on `GET /v1/responses/{id}`.
