...
- **Scale-to-zero:** set `backgroundWorker.autoscaling.replicas.min` to `0`. Keep `activationTargetValue` or `activationLagCount` at `0` so a single queued job activates scaling. Tune `jobsPerReplica` or `lagCount` if brief idle periods flap replicas.
- **Store metrics:** enable `responses-api-store.metrics.enabled=true` (default) so the store Service exposes port `8080` (or `responses-api-store.metrics.port` / `metrics.listenAddr`). When the store subchart is disabled, set `backgroundWorker.autoscaling.metricsUrl` to a KEDA-reachable URL.
...
- `scripts/build-and-load-images.sh` builds or pulls the gateway image (from `../duihua-gateway` or GHCR), builds the background-worker image, loads both into kind, then restarts both Deployments when deployed.
