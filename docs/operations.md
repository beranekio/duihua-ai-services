...
- **Scale-to-zero bootstrap:** when autoscaling uses `replicas.min: 0`, a Helm hook Job creates the stream consumer group before workers scale from zero (the group is otherwise created on first worker startup). KEDA owns Deployment replica counts when autoscaling is enabled; the chart omits `spec.replicas` so Helm upgrades do not reset scaled workers.
